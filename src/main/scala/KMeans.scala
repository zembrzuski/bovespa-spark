  ...
  def main(args: Array[String]): Unit = {
    ...
    for (i <- 1 to 10) {
      ...
      mycentroids = myDataset
        .cartesian(myCentroidsSparkContext) //step 1
        .map(x => (x._1, (x._2, computeEuclidianDistance(x._1._1, x._2)))) //step 2
        .filter(x => !mycentroids.contains(x._1._1)) //step 3
        .reduceByKey((x1, x2) => if (x1._2 < x2._2) x1 else x2) //step 4
        .map(x => (x._2._1, x._1._1)) //step 5
        .combineByKey(  //step 6
